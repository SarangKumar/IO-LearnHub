...
const App = () => {
  ...
  useEffect(() => {
    ...
    console.log("logic to fetch data");
    setPostData(postDataFetch());
  }, []);
    ...
    <div className="m-2 sm:mx-4 sm:my-10 md:mx-20">
      ...
      <section className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3">
        {postData.filter((post) =>
          post.title.toLocaleLowerCase().includes(input.toLocaleLowerCase())
        ).length !== 0 ? (
          postData
            .filter((post) =>
              post.title.toLocaleLowerCase().includes(input.toLocaleLowerCase())
          ...
          <p className="text-slate-900 capitalize">nothing to show</p>
        )}
      </section>
    </div>
  );
